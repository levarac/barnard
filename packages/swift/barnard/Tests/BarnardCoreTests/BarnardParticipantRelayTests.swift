...
final class BarnardParticipantRelayTests: XCTestCase {
  ...
  func testSybilFloodDoesNotEvictRetainedHandles() {
    let (relay, clock, _, _, sink, _) = fixture(); let bytes = container(1, [4])
    XCTAssertEqual(relay.observe(container: bytes, peerHandle: [0]), .accepted)
    for i in 1...32 { XCTAssertEqual(relay.observe(container: bytes, peerHandle: [UInt8(i)]), .duplicate) }
    clock.now = 1
    for i in 100..<200 { XCTAssertEqual(relay.observe(container: bytes, peerHandle: [UInt8(i)]), .duplicate) }
    clock.now = 2
    relay.advance() // epoch 0 decision: the 32 honest handles are still retained -> r >= k, stays silent
    XCTAssertFalse(relay.isServing)
    clock.now = 31_000
    relay.advance() // epoch 1: honest handles (aged at t=0) and the overflow marker (aged at t=1) both expired -> r=0
    clock.now += 15_001
    relay.advance()
  ...
  func testSaturationWindowTracksOverflowNotStickyFlag() {
    ...
    clock.now = 30_000
    relay.advance() // decision epoch 1: overflow (t=29s) is still within T of t=30s -> stays silent
    XCTAssertFalse(relay.isServing)
    clock.now = 60_000
    relay.advance() // decision epoch 2 (past t=59s): both the 32 handles and the overflow marker have expired -> r=0
    clock.now += 15_001
    relay.advance()
    XCTAssertTrue(relay.isServing)
  }
