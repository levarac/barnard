...
public sealed class BarnardRelayVerification {
    ...
    public data class RegistryVerified(val validFromEnin: Long, val validThroughEnin: Long, val relayExpiresAtEnin: Long) : BarnardRelayVerification()
}
...
public class BarnardParticipantRelay(
    ...
    private data class Candidate(val digest: ByteArray, val envelope: ByteArray, var hop: Int, val verifiedAt: Long, val validFrom: Long, val validThrough: Long, val expires: Long)
    ...
    private val handles = linkedMapOf<String, Long>(); private var saturated = false
    ...
    public fun observe(container: ByteArray, peerHandle: ByteArray): BarnardRelayObservationResult {
        ...
        candidates[key]?.let { it.hop = minOf(it.hop, container[1].toInt() and 255); if (selected == key && (container[1].toInt() and 255) > 0) retain(peerHandle, clock.nowMilliseconds()); return BarnardRelayObservationResult.DUPLICATE }
        ...
        candidates[key] = Candidate(digest, envelope, container[1].toInt() and 255, clock.nowMilliseconds(), verified.validFromEnin, verified.validThroughEnin, verified.relayExpiresAtEnin)
    ...
    public fun advance() {
        ...
        candidates.entries.removeIf { current >= it.value.expires }; if (selected != null && !candidates.containsKey(selected)) deselect(); selectIfNeeded(selected == null || now >= pinUntil)
        handles.entries.removeIf { now - it.value >= 30_000 }; var wasActive = false; activeUntil?.let { if (now >= it) { wasActive = true; stopLease() } }
    ...
    private fun selectIfNeeded(force: Boolean) { if (!force) return; val old = selected; selected = candidates.entries.sortedWith(compareBy({ it.value.hop }, { it.value.verifiedAt }, { it.key })).firstOrNull()?.key; pinUntil = clock.nowMilliseconds() + 300_000; if (old != selected) { stopLease(); handles.clear(); saturated = false } }
    private fun retain(handle: ByteArray, now: Long) { val key = handle.hex(); if (key in handles) handles[key] = now else if (handles.size == 32) saturated = true else handles[key] = now }
    ...
    private fun deselect() { stopLease(); selected = null; handles.clear(); saturated = false }
