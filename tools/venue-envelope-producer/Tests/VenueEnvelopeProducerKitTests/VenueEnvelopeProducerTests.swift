...
final class VenueEnvelopeProducerTests: XCTestCase {
  ...
  // MARK: - Cross-language fixture (Kotlin side: BarnardEngineVenueEnvelopeProducerFixtureTest)

  /// Writes the shared fixture `test-vectors/venue-envelope-producer-fixture.txt` that the Kotlin
  /// test in `packages/android/barnard` reads. Runs as a normal test (not a script) so CI's
  /// existing "run the Swift test suite" step is what keeps the fixture current -- see the PR
  /// description for why this is checked in rather than generated at Kotlin test time (Kotlin's
  /// test module has no Swift toolchain to invoke).
  func testWriteCrossLanguageFixture() throws {
    ...
    # BarnardEngineVenueEnvelopeProducerFixtureTest.kt in packages/android/barnard, whose only
